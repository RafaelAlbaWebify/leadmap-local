...
describe("task workflow", () => {
  ...
  it("creates a business task and clears only after success", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify(openTask), { status: 201, headers: { "Content-Type": "application/json" } })
    );
    renderWithClient(<TaskCreate businessId="business-1" />);
    fireEvent.change(screen.getByLabelText("Task title"), { target: { value: "Call decision maker" } });
  ...
  it("retains entered task values after failure", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Task title"), { target: { value: "Review proposal" } });
    fireEvent.change(screen.getByLabelText("Due date"), { target: { value: "2026-08-01" } });
    fireEvent.click(screen.getByRole("button", { name: "Create task" }));
    expect(await screen.findByRole("alert")).toHaveTextContent("Your entered values are retained");
  ...
  it("groups tasks and completes only after an explicit action", async () => {
    const completed = { ...openTask, status: "completed", updated_at: "2026-07-26T13:00:00Z" };
    const fetchMock = vi.spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(new Response(JSON.stringify([openTask]), { status: 200, headers: { "Content-Type": "application/json" } }))
      .mockResolvedValueOnce(new Response(JSON.stringify(completed), { status: 200, headers: { "Content-Type": "application/json" } }))
      .mockResolvedValue(new Response(JSON.stringify([completed]), { status: 200, headers: { "Content-Type": "application/json" } }));
    ...
    const completedRegion = await screen.findByRole("region", { name: "Completed tasks" });
