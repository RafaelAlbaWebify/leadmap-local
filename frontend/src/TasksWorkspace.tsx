    ...
    <section className="task-create" aria-label={label}>
      ...
      {mutation.isError && (
        <div className="notice error" role="alert">
          Task could not be created. Your entered values are retained.
        </div>
      )}
    </section>
  );
  ...
  return (
    <article className="record-card task-card">
      ...
      {task.status === "open" && (
        ...
        >
          {completion.isPending ? "Completing…" : "Mark completed"}
        </button>
      )}
      {completion.isError && (
        <div className="notice error" role="alert">Task could not be completed.</div>
      )}
    </article>
  );
}
...
export function TasksWorkspace() {
  ...
  if (tasks.isPending) {
    return <div className="notice">Loading tasks…</div>;
  }
  if (tasks.isError) {
    return <div className="notice error">Tasks could not be loaded.</div>;
  }
    ...
    <section className="task-workspace" aria-label="Tasks workspace">
      <section aria-label="Open tasks">
        <div className="panel-heading">
          <div><h2>Open tasks</h2><p>{open.length} explicit follow-up actions</p></div>
        </div>
      ...
      <section aria-label="Completed tasks">
        <div className="panel-heading">
          <div><h2>Completed tasks</h2><p>{completed.length} completed actions</p></div>
        </div>
        <div className="card-grid">
          {completed.map((task) => <TaskCard key={task.id} task={task} />)}
          {completed.length === 0 && <div className="empty-state">No completed tasks.</div>}
        </div>
      </section>
    </section>
  );
