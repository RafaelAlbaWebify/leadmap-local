  ...
  return (
    <div className="table-scroll">
      <table>
        <thead>
          <tr>
            <th>Business</th><th>Category</th><th>Area</th>
            <th>Observed</th><th>Freshness</th><th>Status</th>
          </tr>
        </thead>
        <tbody>
          {leads.map((lead) => (
          ...
          {leads.length === 0 && (
            <tr><td colSpan={6} className="empty-state">No persisted leads yet.</td></tr>
          )}
        </tbody>
      </table>
    </div>
...
export function App() {
  ...
  const pageDescription = {
    ...
    Territories: "Manage the geographic areas used to organise discovery and coverage.",
    ...
    <div className="shell">
          ...
          {navigation.map(([label, Icon]) => {
            ...
            return (
              <button
                ...
                key={label}
                disabled={!enabled}
                onClick={() => enabled && setView(label as View)}
              >
                <Icon size={17} /> {label}
      ...
      <main>
        <header className="page-header">
          <div>
            <p className="eyebrow">IRELAND / TERRITORY INTELLIGENCE</p>
            <h1>{view}</h1>
            <p>{pageDescription}</p>
          </div>
          {view === "Overview" && (
            <button className="primary-action" onClick={() => setView("Discovery")}>
            ...
            <section className="workspace-grid">
              <article className="panel map-panel">
                <div className="panel-heading">
                  <div><h2>Territory workspace</h2><p>Persistent geographic configuration</p></div>
                  <button className="secondary-action" onClick={() => setView("Territories")}>Manage</button>
                </div>
                <div className="map-placeholder">
                  <div className="map-message">
                    <MapIcon size={28} />
                    <strong>Geographic layer scheduled next</strong>
                    <span>Territories are now persistent and ready for GeoJSON geometry.</span>
                  </div>
                </div>
              </article>
              <article className="panel intelligence">
                ...
                <p className="body-copy">
                  Load the Ireland starter library to create Galway City and five reusable query groups.
                </p>
                <button className="primary-action full" onClick={() => seed.mutate()} disabled={seed.isPending}>
                  {seed.isPending ? "Loading…" : "Load Ireland starter data"}
                </button>
                {seed.data && <p className="success-text">Starter data ready: {seed.data.total_query_templates} query groups.</p>}
              </article>
            </section>
            <section className="panel table-panel">
              <div className="panel-heading"><div><h2>Recently observed businesses</h2><p>Acquisition date and freshness are retained.</p></div></div>
              <LeadTable leads={dashboard.data.recent_leads} />
            </section>
          </>
        )}
        ...
        {view === "Territories" && (
          <section className="panel page-panel">
            <div className="panel-heading">
              <div><h2>Configured territories</h2><p>{territories.data?.length ?? 0} geographic areas</p></div>
              <button className="secondary-action" onClick={() => seed.mutate()}>Load Ireland starter</button>
            </div>
            <div className="card-grid">
              {(territories.data ?? []).map((item) => (
                <article className="record-card" key={item.id}>
                  <span className="badge neutral">{item.country_code}</span>
                  <h3>{item.name}</h3>
                  <p>{item.administrative_area ?? "No administrative area"}</p>
                  <small>{item.locality ?? "No locality"}</small>
                </article>
              ))}
              {(territories.data ?? []).length === 0 && <div className="empty-state">No territories configured.</div>}
            </div>
          </section>
        )}
            ...
            <article className="panel page-panel">
              ...
              <label>Query group
                <select value={templateId} onChange={(event) => setTemplateId(event.target.value)}>
                  <option value="">Select query group</option>
                  {Array.from(groupedTemplates.entries()).map(([sector, items]) => (
              ...
              <button
                ...
                disabled={!territoryId || !templateId || plan.isPending}
                onClick={() => plan.mutate()}
              >
                Preview search plan
              </button>
            ...
            <article className="panel page-panel">
              ...
              {plan.data && (
                <>
                  ...
                  <div className="notice">Browser capture remains disabled until the Playwright session state machine is implemented and fixture-tested.</div>
                </>
              )}
            </article>
          </section>
        ...
        {view === "Leads" && (
          <section className="panel page-panel">
            <div className="panel-heading"><div><h2>Lead database</h2><p>{leads.data?.length ?? 0} persisted observations</p></div></div>
        ...
        {view === "Exports" && (
          <section className="panel page-panel">
            <div className="panel-heading"><div><h2>Export records</h2><p>Versioned contracts for CRM and other local tools</p></div></div>
            <div className="export-grid">
              <a className="export-card" href="/api/v1/exports/leads.csv">
                <Download size={22} /><strong>CSV export</strong><span>Tabular CRM import</span>
              </a>
              <a className="export-card" href="/api/v1/exports/leads.json">
                <Download size={22} /><strong>JSON export</strong><span>Versioned machine-readable package</span>
              </a>
            </div>
          </section>
        )}

        {(dashboard.isError || territories.isError || templates.isError || leads.isError) && (
          <div className="notice error">The backend is unavailable or returned an invalid response.</div>
        )}
      </main>
    </div>
  );
