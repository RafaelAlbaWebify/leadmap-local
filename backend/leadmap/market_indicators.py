...
def validate_market_indicator_artifact(document: object) -> dict[str, object]:
    ...
    for field in ("dataset_title", "publisher", "source_url", "licence", "published_at", "retrieved_at"):
    ...
    for index, item in enumerate(records):
        ...
        record = dict(item)
        territory_key = _required_text(record.get("territory_key"), f"records[{index}].territory_key")
        indicator_key = _required_text(record.get("indicator_key"), f"records[{index}].indicator_key")
        sector_key = _required_text(record.get("sector_key", "all"), f"records[{index}].sector_key")
        unit = _required_text(record.get("unit"), f"records[{index}].unit")
        value = record.get("value")
        if not isinstance(value, (int, float)) or isinstance(value, bool) or not math.isfinite(float(value)):
            raise MarketIndicatorValidationError(f"records[{index}].value must be finite numeric data.")
        identity = (territory_key, sector_key, indicator_key)
        if identity in identities:
            raise MarketIndicatorValidationError("Duplicate territory, sector and indicator identity.")
    ...
    if checksum is not None and checksum != expected:
        raise MarketIndicatorValidationError("checksum_sha256 does not match the canonical artifact payload.")
    result["checksum_sha256"] = expected
    return result


def install_market_indicator_artifact(document: object, directory: Path) -> dict[str, object]:
    ...
    if target.exists():
        existing = validate_market_indicator_artifact(json.loads(target.read_text(encoding="utf-8")))
...
def load_market_indicator_artifact(directory: Path, checksum_sha256: str) -> dict[str, object]:
    if len(checksum_sha256) != 64 or any(char not in "0123456789abcdef" for char in checksum_sha256):
...
def list_market_indicator_artifacts(directory: Path) -> list[dict[str, object]]:
...
def territory_indicator_values(
    document: dict[str, object], territory_key: str, sector_key: str | None = None
    ...
    return sorted(selected, key=lambda item: (str(item["indicator_key"]), str(item["sector_key"])))
